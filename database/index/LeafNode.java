...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        if (this.getKey(key).isPresent()) {
            throw new BPlusTreeException("Cannot have a duplicate key");
        }
        int d = metadata.getOrder();
        for (int i = 0; i <= this.keys.size(); i++) {
            if (i == this.keys.size() || this.keys.get(i).compareTo(key) > 0) {
                this.keys.add(i, key);
                this.rids.add(i, rid);
                break;
            }
        ...
        if (this.keys.size() > 2 * d) {
            ...
            LeafNode l2 = new LeafNode(metadata, bufferManager, newKeys, newRids, this.rightSibling, treeContext);
            Long pagenum = l2.getPage().getPageNum();
            rightSibling = Optional.of(pagenum);
            sync();
            return Optional.of(new Pair<>(l2.keys.get(0), pagenum));
        }
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        List<DataBox> newKeys = new ArrayList<>();
        List<RecordId> newRids = new ArrayList<>();
        //put a check on whether or not tree is empty
        while (data.hasNext()) {
            Pair<DataBox, RecordId> values = data.next();
            DataBox key = values.getFirst();
            RecordId rid = values.getSecond();
            if (getKey(key).isPresent()) {
                throw new BPlusTreeException("Cannot have a duplicate key");
            }
            if (this.keys.size() ==  fillnum) {
                newKeys.add(key);
                newRids.add(rid);
                break;
            }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        // first find page and then read byte array
        // from there do the inverse of the above operation to acquire the necessary attributes
        // construct a leaf node (non-persisted) from given attributes
        ...
        return new LeafNode(metadata, bufferManager, p, keys, rids, rightSibling, treeContext);
    }
