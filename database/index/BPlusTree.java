...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        typecheck(key);
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.X);
        if (root.get(key).getKey(key).isPresent()) {
            throw new BPlusTreeException("Duplicate key");
        }
        Optional<Pair<DataBox, Long>> splitNode = root.put(key, rid);
        if (splitNode.equals(Optional.empty())) {
            return;
        } else {
            List<DataBox> newKeys = new ArrayList<>();
            List<Long> newChildren = new ArrayList<>();
            newKeys.add(splitNode.get().getFirst());
            newChildren.add(root.getPage().getPageNum());
            newChildren.add(splitNode.get().getSecond());
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.X);
        if (scanAll().hasNext() || !(root instanceof LeafNode)) {
            throw new BPlusTreeException("Bulkloading requires empty tree");
        }
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> splitNode = root.bulkLoad(data, fillFactor);
            if (splitNode.equals(Optional.empty())) {
                return;
            } else {
                List<DataBox> newKeys = new ArrayList<>();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(LeafNode leaf, Iterator<RecordId> iterator) {
        ...
        private void move(){
            if (leaf.getRightSibling().isPresent()) {
                ...
                this.iterator = this.leaf.scanAll();
                if (!this.iterator.hasNext()) {
                    move();
                }
            }
