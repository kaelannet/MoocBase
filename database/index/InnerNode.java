...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        BPlusNode temp =  getChild(childIndex);
        int d = metadata.getOrder();
        Optional<Pair<DataBox, Long>> copy = temp.put(key, rid);

        // Determines if leaf node overloaded
        if (!copy.equals(Optional.empty())) {
            DataBox copyKey = copy.get().getFirst();
            int index = numLessThan(copyKey, keys);
            keys.add(index, copyKey);
            children.add(index + 1, copy.get().getSecond());

            // Checks if InnerNode has overloaded
            if (keys.size() > 2 * d) {
                List<DataBox> newKeys = new ArrayList<>();
                List<Long> newChildren = new ArrayList<>();

                // split the keys into two sets of d size (middle key removed)
                int j = d + 1;
                while (this.keys.size() > j) {
                    newKeys.add(keys.remove(j));
                    newChildren.add(children.remove(j));
                }
                newChildren.add(children.remove(j));
                DataBox parentKey = keys.remove(d);
                // find where children split point is and then add all children
                // to the right (inclusive) of that point to new InnerNode
                //int splitIndex = children.indexOf(copy.get().getSecond());
                //for (int i = splitIndex; i < children.size(); i++) {

                //}
                // create new right InnerNode and return it's page number and the key to be moved up
                InnerNode l2 = new InnerNode(metadata, bufferManager, newKeys, newChildren, treeContext);
                Long pagenum = l2.getPage().getPageNum();
                sync();
                return Optional.of(new Pair<>(parentKey, pagenum));
            }
        }
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int d = metadata.getOrder();
        while (this.keys.size() <= 2 * d) {
            Optional<Pair<DataBox, Long>> splitNode = getChild(children.size() - 1).bulkLoad(data, fillFactor);
            if (splitNode.equals(Optional.empty())) {
                break;
            }
            keys.add(splitNode.get().getFirst());
            children.add(splitNode.get().getSecond());
        }
        if (keys.size() > 2 * d) {
            List<DataBox> newKeys = new ArrayList<>();
            List<Long> newChildren = new ArrayList<>();
