...
public class Database implements AutoCloseable {
    ...
    void lockTableMetadata(String tableName, LockType lockType) {
        ...
        LockContext metadataRow = getTableInfoContext().childContext(rid.getPageNum());
        LockUtil.ensureSufficientLockHeld(metadataRow, lockType);
    }
    ...
    void lockIndexMetadata(String indexName, LockType lockType) {
        ...
        LockContext indexRow = getIndexInfoContext().childContext(rid.getPageNum());
        LockUtil.ensureSufficientLockHeld(indexRow, lockType);
    }
    ...
    private class TransactionContextImpl extends AbstractTransactionContext {
        ...
        @Override
        public void close() {
            // close locks held by transaction in proper order using LockContext.release();
            try {
                List<Lock> locks = lockManager.getLocks(this);
                for (int i = locks.size() - 1; i > -1; i--) {
                    Lock rLock = locks.get(i);
                    LockContext context = LockContext.fromResourceName(lockManager, rLock.name);
        ...
        private Pair<String, BPlusTreeMetadata> resolveIndexMetadataFromName(String tableName,
            ...
            Database.this.lockIndexMetadata(indexName, LockType.S);
        ...
        public Table getTable(String tableName) {
            ...
            Database.this.lockTableMetadata(tableName, LockType.S);
    ...
    private class TransactionImpl extends AbstractTransaction {
        ...
        public void dropAllTables() {
            TransactionContext.setTransaction(transactionContext);
            try {
                LockContext database = lockManager.databaseContext();
                LockUtil.ensureSufficientLockHeld(database, LockType.X);
        ...
        public void dropIndex(String tableName, String columnName) {
            ...
            TransactionContext.setTransaction(transactionContext);
            try {
                LockContext index = getIndexContext(indexName);
                lockIndexMetadata(indexName, LockType.X);
                LockUtil.ensureSufficientLockHeld(index, LockType.X);
