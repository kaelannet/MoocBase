...
public class GraceHashJoin {
    ...
    private void partition(HashPartition[] partitions, Iterator<Record> records, boolean left,
        ...
        while (records.hasNext())  {
            ...
            DataBox dataValue = record.getValues().get(columnIndex);
            int hashcode = wrap(hashFunc.apply(dataValue), partitions);
            if (left) {
                partitions[hashcode].addLeftRecord(record);
            } else {
                partitions[hashcode].addRightRecord(record);
            }
        }
        return;
    }

    private int wrap(int x, HashPartition[] partitions) {
        int mod = x % partitions.length;
        if (mod < 0) {
            mod += partitions.length;
        }
        return mod;
    }
    ...
    private List<Record> buildAndProbe(HashPartition partition) {
        ...
        HashMap<DataBox, List<Record>> buildTable = new HashMap<>();
        
        // Build stage
        while (buildRecords.hasNext()) {
            Record buildRecord = buildRecords.next();
            DataBox buildKey = buildRecord.getValues().get(buildColumnIndex);
            if (buildTable.containsKey(buildKey)) {
                buildTable.get(buildKey).add(buildRecord);
            } else {
                List<Record> records = new ArrayList<>();
                records.add(buildRecord);
                buildTable.put(buildKey, records);
            }
        }
        ...
        while (probeRecords.hasNext()) {
            ...
            DataBox probeKey = probeRecord.getValues().get(probeColumnIndex);
            // 
            if (buildTable.containsKey(probeKey)) {
                for (Record r : buildTable.get(probeKey)) {
                    Record joinRecord = joinRecords(r, probeRecord, false);
    ...
    private List<Record> run(Iterator<Record> leftRecords, Iterator<Record> rightRecords, int pass) {
        ...
        for (HashPartition partition : partitions) {
            ...
            try {
                for (Record r : buildAndProbe(partition)) {
                    joinedRecords.add(r);
                }
            // Recursive hashing
            } catch (IllegalArgumentException e) {
                for (Record r : run(partition.getLeftIterator(), partition.getRightIterator(), pass + 1)) {
                    joinedRecords.add(r);
                }
            }
        }
    ...
    public static Pair<List<Record>, List<Record>> getBreakNHJInputs() {
        ...
        int usableBuffers = 4;
        int recordsPerPage = 976;
        for (int i = 0; i < (usableBuffers  * recordsPerPage) + 1; i++) {
            leftRecords.add(createRecord(8));
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        for (int i = 0; i < buffers * recordsPerPage; i++) {
            leftRecords.add(createRecord(8));
            rightRecords.add(createRecord(8));
        }
