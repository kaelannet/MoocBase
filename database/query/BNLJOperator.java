...
class BNLJOperator extends JoinOperator {
    ...
    private class BNLJIterator extends JoinIterator {
        ...
        private void fetchNextRecord() {
            ...
            while (true) {
                // Out of records in right page
                if (!rightRecordIterator.hasNext()) {
                    // Out of records in left block
                    if (!leftRecordIterator.hasNext()) {
                        // The right page was at the end so reset rightIterator
                        // And grab the first right page and next left block
                        if (!rightIterator.hasNext()) {
                            fetchNextLeftBlock();
                            if (leftRecord != null) {
                                rightIterator.reset();
                            ...
                            // If there are no more left pages then "No more records to yield"
                            else  {
                                throw new NoSuchElementException("No more records to yield");
                            }
                        }
                        // Grab the next right page
                        // Reset the left block iterator for the next right page
                        // and set the left record to the start of the block
                        else {
                            fetchNextRightPage();
                            leftRecordIterator.reset();
                            leftRecord = leftRecordIterator.next();
                        }
                    // There are more left block records so reset right record
                    // iterator and update the leftRecord to the next one
                    } else {
                ...
                if (rightRecord.equals(leftRecord)) {
