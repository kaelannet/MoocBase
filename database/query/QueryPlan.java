...
public class  QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        // Map starting table to it's lowest cost operator

        HashMap<Set, QueryOperator> firstPass = new HashMap<>();
        Set<String> startTable = new HashSet<>();
        startTable.add(startTableName);
        firstPass.put(startTable, minCostSingleAccess(startTableName));
        ...
        for (String table : joinTableNames) {
            ...
            tableName.add(table);
            firstPass.put(tableName, minCostSingleAccess(table));
        }
        ...
        // been joined.
        Map<Set, QueryOperator> prevMap = new HashMap<>(firstPass);
        while (prevMap.size() > 1) {
            prevMap = minCostJoins(prevMap, firstPass);
        }
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        for (int index : getEligibleIndexColumns(table)) {
            PredicateOperator predicate = selectOperators.get(index);
            DataBox value = selectDataBoxes.get(index);
            String column = selectColumnNames.get(index);
            operator = new IndexScanOperator(transaction, table, column, predicate, value);
            cost = operator.getIOCost();
            if (cost < minCost) {
                minOp = operator;
                minCost = cost;
                except = index;
        ...
        for (Set<String> tableSet : prevMap.keySet()) {
            ...
            for (int i = 0; i < joinTableNames.size(); i++) {
                ...
                if (tableSet.contains(leftTable) && !tableSet.contains(rightTable)) {
                   set.add(rightTable);
                   rightOp = pass1Map.get(set);
                   leftOp = prevMap.get(tableSet);
                }

                 /* Case 2. Set contains right table but not left, use pass1Map to
                 * fetch the right operator to access the leftTable. */

                 else if (!tableSet.contains(leftTable) && tableSet.contains(rightTable)) {
                    set.add(leftTable);
                    rightOp = prevMap.get(tableSet);
                    leftOp = pass1Map.get(set);
                }
                 ...
                 */
                 ...
                 set.addAll(tableSet);
                 minJoin = minCostJoinType(leftOp, rightOp, leftCol, rightCol);
                 if (!map.containsKey(set)) {
                     map.put(set, minJoin);
                 } else if (minJoin.getIOCost() < map.get(set).getIOCost()) {
                     map.replace(set, minJoin);
                 }
            }
        }
        return map;
