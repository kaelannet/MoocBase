...
public class SortOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        PriorityQueue<Pair<Record, Integer>> sorter = new PriorityQueue<>(new RecordPairComparator());
        Iterator<Run> runsIterator = runs.iterator();
        Run mergeSorted = createRun();
        // helps keep track of non empty record iterators in runs
        List<Iterator<Record>> listofiterators = new ArrayList<>();

        int i = 0;
        // Goes through each run and adds pair<first record of run, run #> to sorter
        while (runsIterator.hasNext()) {
            Run run = runsIterator.next();
            Iterator<Record> runIterator = run.iterator();
            // Add first record of each run in runs to sorter
            if (runIterator.hasNext()) {
                Pair<Record, Integer> pair  = new Pair<>(runIterator.next(), i);
                listofiterators.add(runIterator);
                sorter.add(pair);
            }
            i++;
        }
        ...
        while (!sorter.isEmpty()) {
            Pair<Record, Integer> pairOut = sorter.remove();
            Record record = pairOut.getFirst();
            int index = pairOut.getSecond();
            mergeSorted.addRecord(record.getValues());
            Iterator<Record> iter = listofiterators.get(index);
            if (iter.hasNext()) {
                Pair<Record, Integer> pairIn = new Pair<>(iter.next(), index);
                sorter.offer(pairIn);
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int numFreeBuffers = numBuffers - 1;
        int i = 0;
        while (i  < runs.size()) {
           List<Run> mergeRun = runs.subList(i, i + numFreeBuffers);
           mergedRuns.add(mergeSortedRuns(mergeRun));
           i += numFreeBuffers;
        }
        return mergedRuns;
    }
